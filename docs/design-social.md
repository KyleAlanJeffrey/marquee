...
request and its own row — no code path publishes a log entry); this is purely
the presentation collapsing into one flow.
...
4. **Review likes.** One table (`review_likes`), one count, one heart. It
   feeds "popular reviews" ranking on event/artist pages, gives the feed a
   verb, and later gives lists their reaction (which is the trigger the 0016
   migration named for turning list deletion into a tombstone).
...
2. **Mirror artist images on ingest/enrichment.** Content-addressed key
   ...
   upstream URL. A dead Spotify URL stops blanking heroes. Same treatment for
   venue photos (Wikimedia asks for exactly this — hotlinking is discouraged)
   and Clerk avatars on profile sync.
